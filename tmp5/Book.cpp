...
void add_book(vector<Book>& library) {
	...
	printf_s("请输入您要添加的书的名字（不带书名号）：");
	cin.clear();
	cin.ignore();
	//清空缓冲区
	getline(cin, bookname);

	printf_s("请输入您要添加的书的价格：");
	if (!(cin >> price)) {
		printf_s("输入非法，填加失败\n");
		cin.clear();
		cin.ignore();
...
void Book::borrow() {
	if (number > 0) {
		--number;
		printf_s("借出成功，《%s》的当前库存为：%d\n", bookname.c_str(), number);
	}
	else printf_s("借出失败，《%s》的当前库存为：%d\n", bookname.c_str(), number);
}

void Book::restore() {
	++number;
	printf_s("存入成功，《%s》的当前库存为：%d\n", bookname.c_str(), number);
}